...
class VTCRT_Checkbox_classes{
    ...
    */
    public function vtcrt_produce_tax_checked_list ($post_ID, $taxonomy) { 
    ...
    */
    public function vtcrt_process_tax_checklist ($post_ID, $taxonomy) { 
        $checked_list = wp_get_object_terms($post_ID, $taxonomy, array('fields'=>'ids'));
        return $checked_list;
    }
    ...
    public function vtcrt_is_product_in_tax_list ($prod_id, $tax_name, $checked_list) { 
    ...
    *                           
   */
    public function vtcrt_fill_roles_checklist ($tax_class, $checked_list = NULL) { 
    ...
    */
    public function vtcrt_process_roles_checklist () { 
    ...
    */
    public function vtcrt_checked_list_from_checkboxes ($checkbox) { 
        $checked_list;
        for($i=0; $i < sizeof($checkBox); $i++){
            $checked_list .= $checkBox[$i].'>';   //returns 'value' from checkbox input statement
        } 
    ...
    public function vtcrt_is_user_in_role_list ($checked_list) { 
        if ( !is_user_logged_in() ) {
            return false;
        }
        if (strpos($checked_list, vtcrt_get_current_user_role() )) {   //if role is in previously checked_list
          return true;
        } else { 
          return false;
        }
    ...
    public function vtcrt_get_current_user_role() {
      	global $current_user;     
